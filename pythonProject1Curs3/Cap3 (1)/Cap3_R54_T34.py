...
# sau utilizand  functia eval: bd = [eval('t'+ str(x)) for x in range(1, 16)]
print(bd)
...
dd = {cheie: valoare for cheie, valoare in enumerate(bd, start = 1)}
...
# medici pediatrie
medici_spec = []
def med_spec(specialitate):
    medici_spec = []
    for i in bd:
        if i[3] == specialitate:
            medici_spec.append(i)
    return medici_spec
...
for i in bd:
    #print (i)
    if i[3] == 'neurologie' or i[3] == 'psihiatrie':
        for elem in i:
